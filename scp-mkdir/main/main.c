...
void app_main(void)
{
	...
	if (stat != 0) {
		rc = libssh2_sftp_mkdir(sftp_session, CONFIG_SSH_PATH,
							LIBSSH2_SFTP_S_IRWXU|
							LIBSSH2_SFTP_S_IRGRP|LIBSSH2_SFTP_S_IXGRP|
							LIBSSH2_SFTP_S_IROTH|LIBSSH2_SFTP_S_IXOTH);
		if(rc) {
			ESP_LOGE(TAG, "libssh2_sftp_mkdir failed: %d", rc);
		} else {
			ESP_LOGI(TAG, "%s created", CONFIG_SSH_PATH);
		}
	} else {
		ESP_LOGW(TAG, "%s is already exist", CONFIG_SSH_PATH);
	}
